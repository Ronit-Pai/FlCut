...
describe("getLinkStatus", () => {
  test("ACTIVE  — plain link with no dates", () =>
    expect(getLinkStatus(link(), NOW)).toBe("ACTIVE"));

  test("SCHEDULED — goLiveAt is future, no expiry", () =>
    expect(getLinkStatus(link({ goLiveAt: FUTURE }), NOW)).toBe("SCHEDULED"));

  test("ACTIVE    — goLiveAt is past (already went live)", () =>
    expect(getLinkStatus(link({ goLiveAt: PAST }), NOW)).toBe("ACTIVE"));

  test("EXPIRED   — expiresAt is past", () =>
    expect(getLinkStatus(link({ expiresAt: PAST }), NOW)).toBe("EXPIRED"));

  test("ACTIVE    — expiresAt is future", () =>
    expect(getLinkStatus(link({ expiresAt: FUTURE }), NOW)).toBe("ACTIVE"));

  test("DISABLED  — status=DISABLED overrides all dates", () =>
    expect(
      getLinkStatus(link({ status: "DISABLED", goLiveAt: FUTURE, expiresAt: FAR_FUTURE }), NOW),
    ).toBe("DISABLED"));

  test("SCHEDULED — both goLiveAt and expiresAt in future", () =>
    expect(
      getLinkStatus(link({ goLiveAt: FUTURE, expiresAt: FAR_FUTURE }), NOW),
    ).toBe("SCHEDULED"));

  test("EXPIRED takes priority over SCHEDULED (expiresAt past, goLiveAt future)", () =>
    expect(
...
describe("isActive", () => {
  ...
  test("false — scheduled",             () => expect(isActive(link({ goLiveAt: FUTURE }), NOW)).toBe(false));
  test("false — expired",               () => expect(isActive(link({ expiresAt: PAST }), NOW)).toBe(false));
  test("false — disabled",              () => expect(isActive(link({ status: "DISABLED" }), NOW)).toBe(false));
});
...
describe("getCountdownLabel", () => {
  ...
  test("null — active link with no expiry", () =>
    expect(getCountdownLabel(link(), NOW)).toBeNull());

  test("null — disabled link", () =>
    expect(getCountdownLabel(link({ status: "DISABLED" }), NOW)).toBeNull());

  test("null — disabled link ignores goLiveAt", () =>
    expect(
      getCountdownLabel(link({ status: "DISABLED", goLiveAt: FUTURE }), NOW),
    ).toBeNull());
});
